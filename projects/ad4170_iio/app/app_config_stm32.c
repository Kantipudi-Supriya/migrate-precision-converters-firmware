...
void ad4170_spi_dma_rx_cplt_callback(DMA_HandleTypeDef* hdma)
{
	...
	if (!dma_cycle_count) {
		...
		dma_buf_current_idx = dma_buf_start_idx + (BYTES_PER_SAMPLE * num_of_active_channels);
	} else {
		memcpy((void*)iio_buf_current_idx, dma_buf_current_idx, rxdma_ndtr / 2);
...
#if (DATA_CAPTURE_MODE == BURST_DATA_CAPTURE)
	dma_buf_start_idx = (uint8_t*)local_buf + (BYTES_PER_SAMPLE * num_of_active_channels);
#else
